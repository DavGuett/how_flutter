import 'package:flutter/material.dart';
import 'package:how_flutter/main.dart';
import 'package:how_flutter/models/cliente.dart';
import 'package:how_flutter/models/funcionario.dart';
import 'package:how_flutter/views/edit_client.dart';
import 'package:how_flutter/views/edit_employee.dart';
import 'package:how_flutter/widgets/drawer_options.dart';

class ClientList extends StatelessWidget {
  ClientList({super.key});
  // Lista de funcionários para preencher a listagem inicialmente, já que a persistência de dados ainda não foi implementada.
  final _clientesStream = supabase.from('cliente').stream(primaryKey: ['id']);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: DrawerOptions(),
      appBar: AppBar(
        title: const Text("Flutter"),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.of(context).pushNamed('registrarCliente');
        },
        child: const Icon(Icons.add),
      ),
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: _clientesStream,
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return const Center(child: CircularProgressIndicator());
          }
          final clientes = snapshot.data!;
          return ListView.builder(
              itemCount: clientes.length,
              itemBuilder: ((context, index) {

                final clienteSupabase = clientes[index];
                final cliente = Cliente();
                cliente.Id = clienteSupabase['id'];
                cliente.Nome = clienteSupabase['nome'];
                cliente.Email = clienteSupabase['email'];
                return ListTile(
                  title: Text(cliente.Nome),
                  subtitle: Text(cliente.Email),
                  trailing: Container(
                    width: 100,
                    child: Row(
                      children: [
                        IconButton(
                            onPressed: () {
                              Navigator.push(context,
                                  MaterialPageRoute(
                                      builder: (context) => ClientEditing(cliente: cliente)));
                            },
                            color: Colors.amberAccent,
                            icon: Icon(Icons.edit)),
                        IconButton(
                            onPressed: () async {
                              await supabase.from('cliente').delete().match({'id': cliente.Id});
                            },
                            color: Colors.red,
                            icon: Icon(Icons.delete))
                      ],
                    ),
                  ),
                );
              }));
        },
      ),
    );
  }
}
